...
class Earthquake():
    ...
    def calc_tpmax(self, window_length=4, start_window=0, filter_limits=[0.1, 19], filter_corners=3, blank_time=0.0):
        """
        ...
        filter_limits : [int, int], optional
            DESCRIPTION. The default is [0.1,19]. FIRST VALUE IS FOR HIGHPASS ACC-->VEL, SECOND IS FOR LOWPASS (USED IN ALL)
    ...
    def calc_tc(self, window_length=4, start_window=0, blank_time=0.0):
        """
            ...
            for i in range(0, len(data)):
                if data[i].stats.channel[2] == 'Z':
                    ...
                    if tr_name in picks.keys():
                        ...
                        numerator = vel_for_tc[int(start + blank_time * sampling_rate):int(end) + 1] ** 2
                        numerator = np.trapz(numerator)  # .integrate()

                        denominator = displ[int(start + blank_time * sampling_rate):int(end) + 1] ** 2
    ...
    def calc_distance(self, tr):
        ...
        distance = np.sqrt(
            ...
            + (self.event_stats['eq_long'] - sta_long)**2) * 110
        return distance

    def calc_iv2(self, window_length=4, subtract_bkg=True, filter_limits=[0.075, 10], blank_time=0.0):
        """
        ...
        def actual_iv2_calculation(tr, pick, window, bkg, blank_time=0.0):
            """
            ...
            start = int(start + blank_time * sampling_rate)
            vel = tr.copy()
            v2 = vel.copy()
            if bkg is True:
            ...
            for i in range(0, len(data_interp)):  # iterate through all traces
                ...
                if data_interp[i].stats.channel[2] == 'Z' and tr_name in picks.keys():  # only use vertical components at stations with a pick
                    ...
                    try:
                        ...
                        sampling_rate = trace.stats.sampling_rate
                        if distance < 100:
                            iv2 = actual_iv2_calculation(trace, pick, window_length, subtract_bkg, blank_time=blank_time)
    ...
    def calc_pgd(self, window_length=1, blank_time=0.0):
        def calc_distance(tr):
            """
                ...
                if data[i].stats.channel[2] == 'Z':  # only use vertical components
                    ...
                    if tr_name in picks.keys():
                        ...
                        if snr > 20:
                            ...
                            pgd_value.append(max(pgd_timeseries[int(pick_samples+blank_time*sampling_rate):int(pick_samples + window_length * sampling_rate)]))
