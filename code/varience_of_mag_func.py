...
def plot_data_subplots_line(x_list, y_list, types, f, tp_params, pgd_params, tc_params, iv2_params, n = 0, save = False, show = True):
    ...
    for i in range(0, 4):
        ...
        axs[row][col].tick_params(axis='both', which='major', labelsize=12)
        #axs[row][col].set_xticks([3,4,5,6,7,8], zorder = 110)
        x_unique = np.arange(min(x_list[i]),max(x_list[i]), 0.1)
        from scipy.optimize import curve_fit
        def func(x, a, b):
            return a*x+b

        popt, pcov = curve_fit(func, x_list[i], y_list[i])
        a = popt[0]
        b = popt[1]
        #a = params[i][0][0]
        #std_a = params[i][1][0]
        #b = params[i][2][0]
        #std_b = params[i][3][0]
        median, bin_edges, bin_number = scipy.stats.binned_statistic(y_list[i], x_list[i], bins=np.arange(2.95,8.05,0.1),statistic='median', range=None)
        axs[row][col].scatter( median, bin_edges[:-1]+0.05, marker = 'o', facecolor = 'lightgrey', edgecolors = colors[types[i]], zorder = 1000)
        #y_min, y_max = calc_min_max(1, a, b, std_a, std_b, x_unique = x_unique)
        #axs[row][col].fill_between(x_unique, y_min, y_max, color = '#003f5c', alpha = 0.1, zorder = 100)#, label = '1sd')
        #y_min, y_max = calc_min_max(2, a, b, std_a, std_b, x_unique  = x_unique)
        #axs[row][col].fill_between(x_unique, y_min, y_max, color = '#003f5c', alpha = 0.2, zorder = 99)#, label = '2sd')
        if b >=0:
            axs[row][col].plot(x_unique, a*x_unique+(b), marker = 'x',color='#003f5c',zorder=102,label=f'M3+: {a:.2f}M+{(b-5*a):.2f}')
        else:
            axs[row][col].plot(x_unique, a*x_unique+(b), marker = 'x',color='#003f5c',zorder=102,label=f'M3+: {a:.2f}M-{abs((b-5*a)):.2f}')
        
        popt, pcov = curve_fit(func, bin_edges[:-1]+0.05, median)
        a = popt[0]
        b = popt[1]
        #a = params[i][0][0]
        #std_a = params[i][1][0]
        ...
        #axs[row][col].fill_between(x_unique, y_min, y_max, color = '#003f5c', alpha = 0.2, zorder = 99)#, label = '2sd')
        if b >=0:
            axs[row][col].plot(x_unique, a*x_unique+(b), marker = '^',color='#003f5c',zorder=102,label=f'M3+: {a:.2f}M+{(b-5*a):.2f}')
        else:
            axs[row][col].plot(x_unique, a*x_unique+(b), marker = '^',color='#003f5c',zorder=102,label=f'M3+: {a:.2f}M-{abs((b-5*a)):.2f}')
                
        #x_unique = np.arange(window_lengths[time],8,0.1)
        axs[row][col].set_xlim([x_unique[0],x_unique[-1]])
    ...
    figure.tight_layout()
    if save == True:
        plt.savefig(f'/home/earthquakes1/homes/Rebecca/phd/seismo_det/figures/flipped_data_subplots/{time}_blankwindow_{name_to_blank(f)}_snr{snr}_n{n}.pdf')
    if show == True:
        plt.show()
    else:
        plt.close()