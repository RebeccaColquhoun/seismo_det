...
if hypo and not laptop:
    ...
    save_path = '/home/earthquakes1/homes/Rebecca/phd/seismo_det/figures/overlaps_reversed/hypocentral'
elif not hypo and not laptop:
    data_path = '/home/earthquakes1/homes/Rebecca/phd/data/results_database'
    save_path = '/home/earthquakes1/homes/Rebecca/phd/seismo_det/figures/overlaps_reversed/epicentral'
...
def test_overlap_subplots(params):
    ...
    for i, p in enumerate(params):
        ...
        if p[0][0] >= p[0][flip]-p[1][flip] and p[0][0] <= p[0][flip]+ p[1][flip]:
            print('overlap at 1 s.d.')
            print(f'{p[0][0]} in range [{p[0][flip]-p[1][flip]}, {p[0][flip]+p[1][flip]}]')
            marker = marker + '1_'
        else:
            marker = marker + '0_'
        if p[0][0] >= p[0][flip]-2*p[1][flip] and p[0][0] <= p[0][flip]+2*p[1][flip]:
            print('overlap at 2 s.d.')
            print(f'{p[0][0]} in range [{p[0][flip]-2*p[1][flip]}, {p[0][flip]+2*p[1][flip]}]')
            marker = marker + '1_'
        else:
            marker = marker + '0_'
        if p[0][0] <= p[0][flip]-2*p[1][flip] or p[0][0] >= p[0][flip]+2*p[1][flip]:
            print('no overlap at 2 s.d')
            print(f'{p[0][0]} NOT in range [{p[0][flip]-2*p[1][flip]}, {p[0][flip]+2*p[1][flip]}]')
