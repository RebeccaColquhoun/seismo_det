...
def plot_iv2(title, mag_lim, corr_df, corr_type = '2'):
    ...
    plt.savefig(f'/home/earthquakes1/homes/Rebecca/phd/seismo_det/figures/all_data/iv2/{title}/mag{mag_lim:.1f}_r{corr_type}.pdf', format = 'pdf')
    plt.close()
...
for f in filenames:
    ...
    for mag_lim in magnitudes:
        ...
        if len(set(df2['mag'])) > 1:
            plot_iv2(f, mag_lim, df2, corr_type = '2')
        if len(set(df138['mag'])) > 1:
            plot_iv2(f, mag_lim, df138, corr_type = '138')
