...
parameters = [
[0.75,0.1,19,0,'eq_object_075s_bandpass_01_19_snr_20_blank_0'],
[0.75,0.1,19,0.05,'eq_object_075s_bandpass_01_19_snr_20_blank_005'],
[0.75,0.1,19,0.1,'eq_object_075s_bandpass_01_19_snr_20_blank_01'],
[0.75,0.1,19,0.25,'eq_object_075s_bandpass_01_19_snr_20_blank_025'],
...
def find_with_data(root_path, cat):
    eq_with_data = []
    cat_with_data = obspy.Catalog()  # cat.copy()
    # cat_with_data.clear()
    for event in cat:  # check earthquakes have data AND PICKS
        eq_name = util.catEventToFileName(event)
        if os.path.isdir(root+eq_name) and os.path.isdir(root_path+eq_name+'/station_xml_files') and os.path.exists(root+eq_name+'/picks.pkl'):
            eq_with_data.append(eq_name)
            cat_with_data.extend([event])
    return eq_with_data, cat_with_data
            
def do_calculation_new(eq_with_data, cat_with_data, root_path):        
    print("let's begin")
    for params in parameters:
        ...
        for eq_no in range(0, len(eq_with_data)):
            ...
            print('make object')
            eq = earthquake.Earthquake(eq_with_data[eq_no], cat_with_data[eq_no], root = root_path)
            eq.eq_info()
            eq.load(root=root)
